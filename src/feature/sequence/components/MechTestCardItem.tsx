...
export const MechTestCardItem = ({
    ...
    const controlMode = cardData.control;
    const target = cardData.target;
    const time = cardData.time;
    const velocity = cardData.velocity;
    const dispToggle = cardData.dispToggle;
    const upper = cardData.upper;
    const lower = cardData.lower;
    const frequency = cardData.frequency;
    const profileID = cardData.profileID;
    const imgMode = cardData.imgMode;
    ...
    const renderCardHeaderSummary = (): React.ReactNode => {
        if (type === 'ramp') {
            ...
            const takeProfileID = cardData.take?.data?.profileID;
            const stepType = cardData.step?.type || 'ramp';
            const stepAxis = cardData.step?.data?.axis;
            const stepControl = cardData.step?.data?.control;
    ...
    const cardId = cardIdProp || watch(`${namePrefix}.id` as any) as string || `card-${index}-${namePrefix.replace(/\./g, '-')}`;

    return (
        <MechTestSortableItem id={cardId}>
            {({ ref, handleRef, attributes, listeners, style, isDragging }) => (
                <div 
                    ref={ref}
                    style={style}
                                            ...
                                            <Tooltip delayDuration={200}>
                                                <TooltipTrigger asChild>
                                                    <Button
                                                        ...
                                                        variant="ghost"
                                                        size="icon"
                                                        onClick={() => moveOutOfGroup(index)}
                                                        className="h-8 w-8 text-mauve-400 dark:text-mauve-500 hover:text-mauve-700 hover:bg-mauve-100 dark:hover:bg-mauve-900/30 rounded-lg cursor-pointer transition-colors"
                                                    >
                                                        <LogOut className="h-4 w-4" />
                                                    </Button>
                                                </TooltipTrigger>
                                                <TooltipContent className="max-w-xs text-xs p-2 bg-popover text-popover-foreground rounded shadow-md border border-mauve-150">
                                                    Move step out of group
                                                </TooltipContent>
                                            </Tooltip>
                                        </TooltipProvider>
                                    )}
