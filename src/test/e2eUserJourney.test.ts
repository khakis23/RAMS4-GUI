...
(async () => {
    ...
    // Step 3.5: DIC Profiles Setup
    let createdDicProfileId = "";
    await runTest('User Journey Step 3.5: Create DIC scan profiles (DIC Stills)', () => {
        createdDicProfileId = `dicProfile${Date.now()}`;
        const dicProfilesData = [
            {
                id: createdDicProfileId,
                name: "Optical Surface Strain Scan",
                mode: "stills" as const,
                ctime: 0.1,
                stillPoints: [
                    { ramsx: 15.0, ramsz: -2.5, ome: 90.0, numPoints: 1 }
                ]
            }
        ];
        configStore.updateDraft({ dicProfiles: dicProfilesData });

        const currentDraft = useConfigurationStore.getState().draft;
        assert.strictEqual(currentDraft.dicProfiles.length, 1, "DIC profiles list must contain created profile");
        assert.strictEqual(currentDraft.dicProfiles[0].id, createdDicProfileId, "DIC profile ID must match");
    });
    ...
    await runTest('User Journey Step 5: Build mechanical test sequence and link Take step to DIC profile', () => {
        ...
        assert.strictEqual(currentCards[3].data.profileID, createdDicProfileId, "Take step profileID must link to created DIC profile");
    });
    ...
    await runTest('User Journey Step 7: Reload saved configuration into memory and verify 100% field round-trip accuracy', () => {
        ...
        assert.strictEqual(reloadedDraft.dicProfiles.length, 1, "Reloaded DIC profiles count must match");
        assert.strictEqual(reloadedDraft.dicProfiles[0].id, createdDicProfileId, "Reloaded DIC profile ID must match");
        ...
        assert.strictEqual(reloadedCards[3].data.profileID, createdDicProfileId, "Reloaded DIC Take step profileID must remain linked");
    });
