...
| **Spec Host**                        | | `mech_workflow_config.json` | `spec_host` | The network address of the external SPEC server (e.g. id1a3.classe.cornell.edu:spec). | |
| **Require Spec Enable**              | | `mech_workflow_config.json` | `require_spec_enable` | If enabled, the system requires connection to SPEC to proceed with scans. | |
| **System Name**                      | | `RAMS_CHESS.json` | `system_name` | Name of the mechanical testing system (e.g. RAMS4_CHESS). | |
| **Hostname**                         | | `RAMS_CHESS.json` | `host` | Network IP address of the Aerotech controller. | |
| **Axis Count**                       | | `RAMS_CHESS.json` | `axis_count` | The total number of mechanical axes available on the controller. | |
| **Task Count**                       | | `RAMS_CHESS.json` | `task_count` | The total number of concurrent execution tasks supported by the controller. | |
| **Axis Name**                        | | `RAMS_CHESS.json` | `name` | Physical name designation of the axis (e.g., RT, RB, TEN, A, B). | |
| **Axis Max Velocity**                | | `RAMS_CHESS.json` | `max_velocity` | Maximum safety velocity limit for the axis. | |
| **Axis Max Acceleration**            | | `RAMS_CHESS.json` | `max_acceleration` | Maximum safety acceleration limit for the axis. | |
| **Input Signal Name**                | | `RAMS_CHESS.json` | `name` | Designated name for the analog or digital channel (e.g., LoadA, Strain). | |
| **Input Signal Scale**               | | `RAMS_CHESS.json` | `slope` | Calibration multiplier slope to convert voltage into engineering units. | |
| **Input Signal Offset**              | | `RAMS_CHESS.json` | `intercept` | Calibration y-intercept offset value. | |
| **Input Signal Channel**             | | `RAMS_CHESS.json` | `channel` | Board channel number on the controller for this signal. | |
| **Output Path**                      | | `daq_config.json` | `output_directory` | The destination directory where the acquired data files are saved. | |
